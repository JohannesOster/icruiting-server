...
import {catchAsync, BaseError} from 'adapters/errorHandling';
...
export const FormsAdapter = () => {
  ...
  const exportForm = catchAsync(async (req, res) => {
    const {tenantId} = req.user;
    const {formId} = req.params;
  ...
  const renderHTMLForm = catchAsync(async (req, res) => {
    const {formId} = req.params;
  ...
  const submitHTMLForm = catchAsync(async (req, res) => {
    const {formId} = req.params;
    const form = await db.forms.retrieve(null, formId);
    if (!form) throw new BaseError(404, 'Not Found');
