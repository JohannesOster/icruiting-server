...
import {insertAssessment} from '../db/assessments.db ';
...
describe('assessments', () => {
  describe('POST /assessments', () => {
    ...
    beforeAll(async (done) => {
      ...
      const fakeForm: any = fake.assessmentForm(orgId, jobId);
      ...
      assessment = await Promise.all(promises)
        .then((data: any) => {
          const form = data[0];
    ...
    afterEach((done) => {
      db.none('DELETE FROM assessment').finally(done);
    });
    ...
    it('Returns created assessment entity', async (done) => {
      await request(app)
        .post('/assessments')
        .set('Accept', 'application/json')
        .send(assessment)
      ...
      const {count} = (await db.any(stmt, assessment))[0];
  ...
  describe('GET /assessments?applicant_id', () => {
    let assessment: any;
    beforeAll(async (done) => {
      ...
      Promise.all(promises)
        .then(async (data: any) => {
          ...
          assessment = await insertAssessment({
            applicant_id: applicant.applicant_id,
            submitter_id: process.env.TEST_USER_ID || '',
            form_id: form.form_id,
            submission: {
    ...
    it('Returns 200 json response', (done) => {
      request(app)
        .get('/assessments/' + assessment.applicant_id)
        .set('Accept', 'application/json')
        .expect('Content-Type', /json/)
        .expect(200, done);
    });

    it('Returns inserted assessment', async (done) => {
      ...
      expect(resp.body[0]).toEqual(assessment);

      done();
    });
  });

  describe('PUT /assessments?applicant_id', () => {
    let assessment: any;
    beforeAll(async (done) => {
      await db.none('DELETE FROM assessment');
      await db.none('DELETE FROM form');
    ...
    it('Returns 200 json response', (done) => {
      request(app)
        .put('/assessments/' + assessment.applicant_id)
        .set('Accept', 'application/json')
        .send({})
        .expect('Content-Type', /json/)
        .expect(200, done);
    ...
    it('Returns updated assessment', async (done) => {
      const range = {min: 0, max: 5};
      const newValues = {
        [faker.random.alphaNumeric()]: faker.random.number(range),
        [faker.random.alphaNumeric()]: faker.random.number(range),
      ...
      expect(resp.body.applicant_id).toBe(assessment.applicant_id);
