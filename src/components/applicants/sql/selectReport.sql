	...
	FROM
		...
		FROM 
			(SELECT
				...
				ROUND(AVG(form_field_avg) FILTER (WHERE intent='sum_up' AND enhanced_form_submission_field.job_requirement_id IS NOT NULL) OVER (PARTITION BY enhanced_form_submission_field.job_requirement_id),2) AS avg_job_requirement_score
			FROM 
				(SELECT
					*,
				...
				FROM
					...
					ON form_submission_field_view.form_submission_id = form_submission.form_submission_id) AS form_submission_field
				) AS enhanced_form_submission_field
			LEFT JOIN job_requirement ON job_requirement.job_requirement_id = enhanced_form_submission_field.job_requirement_id
